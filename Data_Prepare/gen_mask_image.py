...
def genMaskImage(targetFileName,saveFolder):
...
def genMaskImage_IG(targetFileName,saveDir):
	saveFolder = saveDir
	os.makedirs(saveFolder,exist_ok=True)
	dom = xml.dom.minidom.parse(targetFileName)
	root = dom.documentElement
	...
	for t in images:
		...
		for t_plg in t_polygon:
			...
			if 'Internal_graining' in t_plg_label:
				draw.polygon(t_xy, fill='white')
				draw.line(t_xy, fill='white', width=2)
		...
		for t_pl in t_polyline:
			...
			if 'Internal_graining' in t_pl_label:
				draw.polygon(t_xy, fill='white')
				draw.line(t_xy, fill='white', width=2)
			# t_pl_occluded = t_pl.getAttribute("occluded")

		if t_img.getextrema()[0][1]>0:
			t_img.save(os.path.join(saveFolder, t_name[:11]+'-mask.png'))
			print(t_name[:11] + ' saved ...')


def mkbdySingleType(targetFolderName,targetFileName,saveDir,inclusion_type):
	saveFolder = saveDir
	os.makedirs(saveFolder,exist_ok=True)
	dom = xml.dom.minidom.parse(targetFileName)
	root = dom.documentElement
	...
	for t in images:
		...
		for t_plg in t_polygon:
			...
			t_xy = [(x,y) for x,y in zip(t_x,t_y)]
			t_plg_label = t_plg.getAttribute("label")
			if inclusion_type+'_Reflection' in t_plg_label:
				draw.polygon(t_xy, fill='green')
				draw.line(t_xy, fill='green', width=2)
			elif inclusion_type in t_plg_label:
				draw.polygon(t_xy, fill='red')
				draw.line(t_xy, fill='red', width=2)
			# t_plg_occluded = t_plg.getAttribute("occluded")
		...
		for t_pl in t_polyline:
			...
			t_xy = [(x,y) for x, y in zip(t_x, t_y)]
			t_pl_label = t_pl.getAttribute("label")
			if inclusion_type+'_Reflection' in t_pl_label:
				draw.polygon(t_xy, fill='green')
				draw.line(t_xy, fill='green', width=2)
			elif inclusion_type in t_pl_label:
				draw.polygon(t_xy, fill='red')
				draw.line(t_xy, fill='red', width=2)
			# t_pl_occluded = t_pl.getAttribute("occluded")

		if t_img.getextrema()[0][1]>0 or t_img.getextrema()[1][1]>0 or t_img.getextrema()[2][1]>0:
			t_img_np = np.array(t_img)
			...
			img_np = np.array(img)
			# print(img_np.shape,t_img_np[:,:,0].shape)
			img_np_mask = mkbdy(img_np,t_img_np[:,:,0],color=(1,0,0),outline_color=(1,0,0))
			img_np_mask = mkbdy(img_np_mask,t_img_np[:,:,1],color=(0,1,0),outline_color=(0,1,0))
			img_double = Image.fromarray((img_np_mask*255).astype('uint8'))
			img_double.save(os.path.join(saveFolder, t_name[:11]+'-mask.png'))
			print(t_name[:11] + ' saved ...')
			# except:
