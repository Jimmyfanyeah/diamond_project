...
def save_history_graph(history):
    ...
    history_logfile = '%s/%s_%s_%s_%s_%s_ts%d_imw%s_epochs%d_%s.json'%(history_dir, dataset_name,exp_name, cnn, classify_type,training_type,train_size,imw,epochs,finish_time)
    
    with open(history_logfile,'w') as f:
        json.dump(str(history.history), f)                

    # Plot training & validation loss values
    image_lossfile = '%s/%s_%s_%s_%s_%s_ts%d_imw%s_epoch%d_%s_loss.png'%(graph_dir, dataset_name,exp_name, cnn, classify_type,training_type,train_size,imw,epochs,finish_time)
    ...
    if not is_1vs1:
        image_lossfile = '%s/%s_%s_%s_%s_%s_ts%d_imw%s_epoch%d_%s_acc.png'%(graph_dir, dataset_name,exp_name, cnn, classify_type,training_type,train_size,imw,epochs,finish_time)
...
def train_data():
    ...
    if not is_train:
        ...
        if cnn=='resnet':
            base_model = ResNet50(weights=weight, include_top =False,input_tensor=Input(shape=(imw,imw,3)))
            x = base_model.output
            x = GlobalAveragePooling2D()(x)
        
        else:  #inception
            base_model = InceptionV3(weights=weight, include_top =False,input_tensor=Input(shape=(imw,imw,3)))
            x = base_model.output        
        ...
        if is_1vs1:        
            ...
            model.compile(loss=ovo_crossentropy, optimizer=opt,metrics=[ovo_crossentropy])
            checkpoint = ModelCheckpoint(filepath=filepath, monitor='val_loss', verbose=1, save_best_only=False)  
            lr_scheduler = LearningRateScheduler(lr_schedule)
            callbacks = [checkpoint,lr_scheduler]
            model.summary()
            ...
            all_accuracy = evaluate_1vs1_model(savedmodel_dir,model, x_test,y_test,mapping,trainTargets_ytest,num_classes)
            ...
            all_accuracy = evaluate_1vsall_model(savedmodel_dir,model,x_test,y_test)
            save_history_graph(history)
    
    #ending if is_train, so either train or just test model, writeTOLogfile
    end_time = time.time()
    total_time = (int)(end_time-start_time)/60.0
    write_to_logfile(logfile_report, total_time, dataset_name,exp_name,train_size,imw,epochs,batch_size, all_accuracy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cnn',            default = 'inception', help='resnet, inception')
    parser.add_argument('--is_finetune',    type=str2bool, default=True)    
    parser.add_argument('--is_1vs1',        type=str2bool, default=True)
    parser.add_argument('--is_da'  ,        type=str2bool, default=True, help='train with data augmentation')
    ...
    parser.add_argument('--is_train',       type=str2bool, default=True, help='train or only testing the model')
    ...
    logfile_report = '%s/%s_%s_%s_%s.log'%(logfile_dir,dataset_name,classify_type,cnn,training_type)
    savedmodel_dir = '/project/ppawara/backup_1vs1_revision/savedmodel/savedmodel_%s_%s_ts%d_%s_%s_%s_%d'%(dataset_name,classify_type,train_size,exp_name,cnn,training_type,imw)
    ...
    x_train,y_train, x_test, y_test  = load_dataset(dataset_name, exp_name, imw, num_classes,train_size,is_1vs1)
