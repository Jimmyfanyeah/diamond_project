...
if __name__=='__main__':
    # Preprocess
    data_path = '/media/hdd/diamond_data/UNet_seg_1class'
    cut_path = os.path.join(data_path,'cut')
    clip_path = os.path.join(data_path,'clip256')
    txt_save_path = os.path.join(data_path,'txt256')
    ...
    # print(f'Clip Path:{clip_path} \n# of samples with target in random = {num_with_target}')
    ...
    # print(f'# of samples with target in center_cut method = {num_with_target}')
    ...
    # save_list(trainList,os.path.join(txt_save_path,'id_train.txt'))
    # save_list(valList,os.path.join(txt_save_path,'id_val.txt'))
    # save_list(testList,os.path.join(txt_save_path,'id_test.txt'))

    """ Define a good validation set V1 """
    clarity_grade_sheet_path = '/media/hdd/diamond_data/clarity_grade_20200908.csv'
    clarity_grade = pd.read_csv(clarity_grade_sheet_path)
    print(len(clarity_grade))

    clarity_dict = {'SI1':[],'SI2':[],'VS1':[],'VS2':[],'VVS1':[],'VVS2':[]}
    for idx in range(len(clarity_grade)):
        clarity_tmp = clarity_grade['Clarity'][idx]
        id_tmp = str(clarity_grade['Item no.'][idx])
        clarity_dict[clarity_tmp].append(id_tmp)

    valList = []
    testList = []
    len_list = [34,9,50,32,23,56]
    seed = 10
    for idx,key in enumerate(clarity_dict.keys()):
        clarity_tmp = clarity_dict[key]
        clarity_tmp.sort()
        random.seed(seed)
        random.shuffle(clarity_tmp)
        valList += clarity_tmp[:len_list[idx]]
        testList += clarity_tmp[-len_list[idx]:]

    imgList = list(set([n[:11] for n in os.listdir(cut_path) if not 'mask' in n and 'png' in n]))
    trainList = list(set(imgList).difference(valList).difference(testList))
    print(f'data path: {cut_path},\nNum of train:{len(trainList)} val:{len(valList)} test:{len(testList)}')
    print(f'some example:{trainList[1:3]}')
    ...
    imgList_random = [n for n in os.listdir(clip_path) if not 'mask' in n and 'png' in n]
    # "and len(n.split('_'))==3" = only random cut
